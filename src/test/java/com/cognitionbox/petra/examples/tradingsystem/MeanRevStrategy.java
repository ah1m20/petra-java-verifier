...
public class MeanRevStrategy implements Runnable{
    ...
    public boolean openSell(){return (decision.start() || decision.closed()) && smaSignal.midAboveSma();} // 2

    public boolean openedBuy(){
        return decision.buyOpened() && smaSignal.midBelowSma();
    } // 1

    public boolean openedSell(){
        return decision.sellOpened() && smaSignal.midAboveSma();
    } // 1

    public boolean close(){
        return (decision.buyOpened() && smaSignal.midAboveSma()) || (decision.sellOpened() && smaSignal.midBelowSma()); // 2
    }
