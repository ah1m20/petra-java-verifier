...
public final class EParser {
    public E parse(MethodDeclaration declaration){
        if (declaration.getBody().isPresent() &&
            ...
            return ParserUtils.invalidE(declaration,"expected body with 1 statement.");
        }
    }
    ...
    private E parse(Expression expression){
        if (expression.isMethodCallExpr()) {
            if (expression.asMethodCallExpr().getArguments().size() == 0 &&
                ...
                return ParserUtils.invalidE(expression,"invalid syntax, not the required format: a.p()");
            }
        } else if (expression.isBooleanLiteralExpr()){
            if (expression.asBooleanLiteralExpr().getValue()){
                return new True();
            } else {
                return ParserUtils.invalidE(expression,"invalid syntax, only true is support.");
            }
            ...
            if (binaryOperator==BinaryOperator.UNKOWN){
                return ParserUtils.invalidE(expression,"binary operator not expected, must be one of &&, ||, ^");
            } else {
                return new EBinary(
            ...
            return ParserUtils.invalidE(expression,"must be call to single boolean method or expression of boolean method calls.");
        }
    }
